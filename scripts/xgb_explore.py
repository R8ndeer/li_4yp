...
MODEL_DIR = '/Users/boting/li_4yp/scripts/exp_models_mega/'                 # Directory to save trained models
...
def load_and_prep_data(csv_path):
    ...
    feature_cols = [c for c in df.columns if c.startswith(('RGB_', 'HSV_', 'LAB_', 'YUV_'))]
...
def main():
    ...
    X_train, X_test, indices_train, indices_test = train_test_split(
        X, X.index, test_size=TEST_SIZE, stratify=targets['Base'], random_state=RANDOM_SEED
    )
