...
OUTPUT_CSV = Path(__file__).resolve().parent.parent.parent / "data/demi_v1/demi_v01_avg.csv"

def main():
    # Read the CSV file
    df = pd.read_csv(INPUT_CSV, dtype={"full_shade": str, "level": str, "tone": str, "shade_family": str})
    ...
    color_columns = ['lab_l', 'lab_a', 'lab_b', 'sRGB_r', 'sRGB_g', 'sRGB_b', 'linear_r', 'linear_g', 'linear_b']
    
    # Group by full_shade and aggregate
    grouped = df.groupby('full_shade').agg({
    ...
    column_order = [
        'full_shade', 'level', 'tone', 'shade_family', 'lab_illuminant',
        'lab_l', 'lab_a', 'lab_b',
    ...
    print(f"Samples reduced by: {len(df) - len(averaged_df)} ({((len(df) - len(averaged_df)) / len(df) * 100):.1f}%)")
    ...
    multi_sample_shades = averaged_df[averaged_df['sample_count'] > 1].sort_values('sample_count', ascending=False)
