...
def auto_crop_white(img: np.ndarray, kernel_size: int = 10) -> np.ndarray:
    ...
    _, thresh = cv2.threshold(
        gray, thresh=240, maxval=255, type=cv2.THRESH_BINARY_INV
    )  # <thresh -> 255 for bbox to detect swatch
    ...
    contours, _ = cv2.findContours(
        expanded_mask, mode=cv2.RETR_EXTERNAL, method=cv2.CHAIN_APPROX_SIMPLE
    )
    if not contours:
...
def mask_specular_and_shadow(
    lab_img: np.ndarray, drop_top=0.05, drop_bottom=0.05
) -> np.ndarray:
...
def get_masked_image(
    lab_img: np.ndarray, rgb_img: np.ndarray, drop_top=0.05, drop_bottom=0.05
) -> np.ndarray:
    ...
    """
    assert (
        lab_img.shape == rgb_img.shape
    ), "lab_img and rgb_img must have the same shape"
