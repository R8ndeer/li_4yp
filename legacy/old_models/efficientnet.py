...
class ShadeEfficientNet(nn.Module):
    def __init__(self, num_classes=[12, 11, 11]):
        ...
        self.base_head = nn.Sequential(
            nn.Dropout(p=self.dropout_rate), nn.Linear(in_features, num_classes[0])
        )

        self.primary_head = nn.Sequential(
            nn.Dropout(p=self.dropout_rate), nn.Linear(in_features, num_classes[1])
        )

        self.secondary_head = nn.Sequential(
            nn.Dropout(p=self.dropout_rate), nn.Linear(in_features, num_classes[2])
        )
